...
- [active] google_drive_local_uploads: do not pass a local filesystem path directly to `google_drive_upload_file`; Zapier treats the path string as plain text and creates a `.txt` file instead of uploading the binary. Use `google_drive_api_request_beta` multipart upload or a true remote file object when a real binary file is required. | trigger=uploading local files to Google Drive via Zapier | source=mistake | first_learned=2026-03-13 | last_updated=2026-03-13
- [active] google_drive_upload_name_extension: when using `google_drive_upload_file` with a remote URL that already ends in a file extension, avoid repeating that extension in `new_name` or be prepared to rename the uploaded file; Zapier can create names like `image.png.png`. | trigger=uploading a remote file URL to Google Drive via Zapier with `new_name` set | source=mistake | first_learned=2026-03-14 | last_updated=2026-03-14
